...
def analyse(csv_file_path):
    ...
    count_success_urls = len(success_urls)
    print(f"Success urls: {count_success_urls} ({((count_success_urls/total_urls) * 100):.2f}%)")
    count_error_urls = len(error_urls)
    print(f"Error urls: {count_error_urls} ({((count_error_urls/total_urls) * 100):.2f}%)")
    
    print()
    print(f"Error breakdown:")
    sorted_error_breakdown = sorted(list(error_breakdown.items()), key=lambda x: x[1], reverse=True)
    for error_type, error_count in sorted_error_breakdown:
        print(f"{error_type}: {error_count} {((error_count/count_error_urls) * 100):.2f}%")
    print()
    
    biggest_orgs = sorted(list(org_breakdown.items()), key=lambda x: x[1], reverse=True)[:10]
    print("Biggest orgs")
    pprint(biggest_orgs)
    biggest_org_resource_count = biggest_orgs[0][1]


def main():
    parser = argparse.ArgumentParser(description="A script to analyse broken link CSVs for data.gov.uk")
