...
def main():
    parser = argparse.ArgumentParser(
        description="A script to filter broken link CSVs for data.gov.uk"
    )
    parser.add_argument("input_file_path", type=str, help="The CSV file to filter")
    parser.add_argument(
        "output_file_path", type=str, help="The file path to write the filtered CSV to"
    )
    parser.add_argument(
        "-e",
        "--exclude",
        action="append",
        type=str,
        help="The column(s)/values to exclude",
    )
    parser.add_argument(
        "-ei",
        "--exclude-substring",
        action="append",
        type=str,
        help="The column(s)/values to exclude if exclusion value is a substring of a particular row's value",
    )
    ...
    for exclude_argument in args.exclude:
        try:
            ...
            raise Exception(
                f"--exclude argument {exclude_argument} did not follow <column>=<value> pattern"
            )
        if "," in value:
            value = value.split(",")
        else:
    ...
    for exclude_argument in args.exclude_substring:
        try:
            ...
            raise Exception(
                f"--exclude-substring argument {exclude_argument} did not follow <column>=<value> pattern"
            )
        if "," in value:
            value = value.split(",")
        else:
    ...
    filter(
        args.input_file_path, args.output_file_path, exclusions, exclusion_substrings
    )
