...
def sort_report(report_path: str) -> None:
    ...
    with open(
        report_path.replace(".csv", "_sorted.csv"), "w", newline="", encoding="utf-8"
    ) as csvfile:
...
def main(argv: list[str] | None = None) -> int:
    ...
    report_files = [
        ...
        if f.startswith("check_links_report_") and f.endswith(".csv")
    ]
    report_path = (
        os.path.join(args.input_dir, report_files[0]) if report_files else None
    )
    ...
    logger.info(
        f"Sorted report file saved to {report_path.replace('.csv', '_sorted.csv')}"
    )

    if os.environ.get("CKAN_OUTPUT_BUCKET_NAME"):
        upload_to_s3(logger, report_path.replace(".csv", "_sorted.csv"))
        logger.info(
            f"Uploaded sorted report to S3 bucket {os.environ.get('CKAN_OUTPUT_BUCKET_NAME')}"
        )
